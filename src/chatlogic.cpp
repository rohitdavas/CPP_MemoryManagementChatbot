...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        std::cout << "Checking if the node with ID already exists" << std::endl; 
                        ...
                        {	
                          	std::cout << "node does not exists. Creaating a new node with unique ... " << std::endl; 
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {	
              	std::cout << "assigning the current node to root ..." << std::endl; 
